...
export function ShareDialog({ url = "https://recording.com", onClose }: ShareDialogProps) {
  ...
  return (
    <>
      ...
      <div className={cn(
        "absolute right-full top-1/2 -translate-y-1/2 mr-2 z-50 px-2 py-1.5 flex items-center gap-2 whitespace-nowrap",
        "bg-widget-dialog",
        "rounded-widget-sm",
        "shadow-[0px_2px_8px_var(--widget-panel-shadow)]"
      )}>
        <p className={cn(
          "font-['Raleway',sans-serif] font-semibold text-[12px]",
          "text-widget-text-primary"
        )}>
          Copy the link
        </p>
        <div className={cn(
          "px-2 py-1 flex items-center gap-2 min-w-[120px]",
          "bg-widget-input",
          "rounded-widget-sm"
        )}>
          <p className={cn(
            "font-['Raleway',sans-serif] font-semibold text-[10px] truncate flex-1",
            "text-widget-text-primary"
          )}>
            {url}
          </p>
          ...
          >
            <div className="size-[18px]">
              <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 25 25">
                <rect fill={copied ? "hsl(var(--widget-status-success))" : "hsl(var(--widget-input-bg))"} height="25" rx="4" width="25" />
                <path 
                  clipRule="evenodd" 
                  ...
                  fill="hsl(var(--widget-icon-primary))" 
                  fillRule="evenodd" 
                />
              </svg>
            </div>
